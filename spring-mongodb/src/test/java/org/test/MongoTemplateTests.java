...
public class MongoTemplateTests extends SpringBootMongodbDemoApplicationTests {
    ...
    public void testSave() {
        User user = new User(5, "ccc", 2100.1, new Date());
    ...
    public void testFind() {
        ...
        //4.等值查询
        mongoTemplate.find(Query.query(Criteria.where("username").is("aaa啊啊啊")), User.class);
        ...
        //6.and 查询
        mongoTemplate.find(Query.query(Criteria.where("name").is("aaa").and("salary").is(2300.1)), User.class);
        ...
        //8. and  or
        mongoTemplate.find(Query.query(Criteria.where("salary").is(2800.1).orOperator(Criteria.where("name").is("ccc"))), User.class);
        ...
        //11.总条数
        long count = mongoTemplate.count(Query.query(Criteria.where("name").is("aaa")), User.class);
        ...
        //13.使用 json 字符串方式查询
        Query query = new BasicQuery("{$or:[{name:'aaa'},{name:'ccc'}]}", "{name:0}");
    ...
    public void testRemove() {
        ...
        //条件删除
        //mongoTemplate.remove(Query.query(Criteria.where("name").is("ddd")), User.class);
    }

    //1.创建集合
    @Test
    public void testCreateCollection() {
        String collectionName = "bicycle";
        //判断集合是否存在
